...
// Floating point implementation of IDCT, DEQUANT,... 
// Matija Čavrag, FER, 2016, matija.cavrag@fer.hr
// version:     3.0         10.6.2016.
...
{ 8, 6, 5, 8, 12, 20, 26, 31 }, { 6, 6, 7, 10, 13, 29, 30, 28 }, { 7, 7, 8, 12,
		20, 29, 35, 28 }, { 7, 9, 11, 15, 26, 44, 40, 31 }, { 9, 11, 19, 28, 34,
		55, 52, 39 }, { 12, 18, 28, 32, 41, 52, 57, 46 }, { 25, 32, 39, 44, 52,
		61, 60, 51 }, { 36, 46, 48, 49, 56, 50, 52, 50 } };

static const unsigned int q_chrom[8][8] = { { 9, 9, 12, 24, 50, 50, 50, 50 }, {
		9, 11, 13, 33, 50, 50, 50, 50 }, { 12, 13, 28, 50, 50, 50, 50, 50 }, {
		24, 33, 50, 50, 50, 50, 50, 50 }, { 50, 50, 50, 50, 50, 50, 50, 50 }, {
		50, 50, 50, 50, 50, 50, 50, 50 }, { 50, 50, 50, 50, 50, 50, 50, 50 }, {
		50, 50, 50, 50, 50, 50, 50, 50 }, };

float32_t IDCTcoeff[8][8][8][8];
...
float32_t YUV[8][8][3] = { { { 0 } } };

uint32_t RGB[8][8][3] = { { { 0 } } };
...
int getDCvalue(int * pos, int comp) {
	...
	for (i = 0; i < 12; i++) {
		...
		mask = (comp == 1) ?
				((1 << dc_lum[i][0]) - 1) : ((1 << dc_chrom[i][0]) - 1);
		...
		for (i = 0; i < 16; i++) {
			...
			for (j = 0; j < 11; j++) {
				...
				mask = (comp == 1) ?
						...
						((1 << ac_chrom[i][j][0]) - 1);
				shift = (comp == 1) ?
						(ac_lum[i][j][0] - 1) : (ac_chrom[i][j][0] - 1);
				huffData =
						(comp == 1) ? (ac_lum[i][j][1]) : (ac_chrom[i][j][1]);
...
void DQuant(int comp) {
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 8; j++) {
			IDCTBlock[i][j] = (float) (DQFBlock[i][j]
					* (float) ((comp == 1) ? q_lum[i][j] : q_chrom[i][j]));
		}
	}
}
...
void IDCT(int comp) {
	...
	float32x4_t IDCTvalues;

	float32x4_t IDCTDataBlock;
	...
	for (x = 0; x < 8; x++) {
		for (y = 0; y < 8; y++) {
			temp = vdupq_n_f32(0.0);
			for (u = 0; u < 8; u++) {
				for (v = 0; v < 8; v += 4) {
					IDCTvalues = vld1q_f32(&IDCTcoeff[x][y][u][v]);
					IDCTDataBlock = vld1q_f32(&IDCTBlock[u][v]);
					temp = vmlaq_f32(temp, IDCTvalues, IDCTDataBlock);
				}
			}

			sum = vgetq_lane_f32(temp,0) + vgetq_lane_f32(temp,1) +vgetq_lane_f32(temp,2)+vgetq_lane_f32(temp,3);

			dataBlock[x][y] = (int32_t) (sum * 0.25);
		}
	}
...
void yuv2rgb() {
	...
	float32_t const1 = 1.402;
	float32_t const2 = 0.344;
	float32_t const3 = 0.714;
	float32_t const4 = 1.772;

	float32x4_t shift = vmovq_n_f32(128.0);


	for (i = 0; i < 8; i++) {
		for (j = 0; j < 8; j += 4) {

			float32x4x3_t yuv = vld3q_f32(&YUV[i][j][0]);

			result.val[0] = vcvtq_u32_f32(
					vaddq_f32(yuv.val[0],
							vmulq_n_f32(vsubq_f32(yuv.val[2], shift), const1)));

			result.val[1] = vcvtq_u32_f32(
					vsubq_f32(yuv.val[0],
							vaddq_f32(
									vmulq_n_f32(vsubq_f32(yuv.val[1], shift),
											const2),
									vmulq_n_f32(vsubq_f32(yuv.val[2], shift),
											const3))));

			result.val[2] = vcvtq_u32_f32(
					vaddq_f32(yuv.val[0],
							vmulq_n_f32(vsubq_f32(yuv.val[1], shift), const4)));


			vst3q_u32(&RGB[i][j][0], result);
	...
	uint32x4_t max = vdupq_n_u32(255);

	uint32x4x3_t res;

	for (i = 0; i < 8; i++) {
			for (j = 0; j < 8; j+=4) {
				uint32x4x3_t clamp = vld3q_u32(&RGB[i][j][0]);
				res.val[0] = vminq_u32(clamp.val[0], max);
				res.val[1] = vminq_u32(clamp.val[1], max);
				res.val[2] = vminq_u32(clamp.val[2], max);

				vst3q_u32(&RGB[i][j][0], res);
			}
	}
...
void storeRGB(int x) {
	...
	fseek(fout, (long) (tmpx * (512 / 8) * 192 + tmpxx * 24 + fileoffset),
	SEEK_SET);
...
void calculateIDCTCoeff() {
	...
	for (x = 0; x < 8; x++)
		for (y = 0; y < 8; y++) {
			for (u = 0; u < 8; u++)
				for (v = 0; v < 8; v++)
					IDCTcoeff[x][y][u][v] += ((u == 0) ? 1. / sqrt(2.) : 1.)
							* ((v == 0) ? 1. / sqrt(2.) : 1.)
							* cos(
...
void main(int argc, char * argv[]) {
	...
	while (input[position] != '\0' && flag == 1) {
		...
		//Get DC and AC values for blocks
